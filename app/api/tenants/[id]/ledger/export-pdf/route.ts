...
export async function GET(
  ...
  for (const inv of invoices) {
    const period = inv.periodStart && inv.periodEnd
      ...
      : "";
    const month  = inv.monthNumber ? ` — Month ${inv.monthNumber}` : "";
    rows.push({
      date:        inv.issueDate,
      type:        "Invoice",
  ...
  const periodLabel    = dateFrom || dateTo
    ? `${dateFrom ? fmtDate(new Date(dateFrom)) : "All"} → ${dateTo ? fmtDate(new Date(dateTo)) : "Today"}`
    : "All time";
...
<title>Tenant Ledger — ${tenantName}</title>
<style>
...
<div class="no-print" style="position:fixed;top:12px;right:16px;z-index:99;display:flex;gap:8px;">
  <button onclick="window.print()" style="background:#2563eb;color:#fff;border:none;border-radius:6px;padding:8px 18px;font-size:13px;font-weight:600;cursor:pointer;box-shadow:0 2px 6px rgba(37,99,235,.35);">Print / Save PDF</button>
  <button onclick="window.close()" style="background:#f3f4f6;color:#374151;border:1px solid #d1d5db;border-radius:6px;padding:8px 14px;font-size:13px;cursor:pointer;">Close</button>
</div>

<!-- Header -->
<div class="header">
  <div class="header-left">
    <h1>Tenant Statement of Account</h1>
    <div class="subtitle">كشف حساب المستأجر</div>
  </div>
  <div class="header-right">
    <div class="org-name">${org?.name ?? "Property Management"}</div>
    <div class="org-sub">${org?.city ?? "Salalah"}, Oman${org?.phone ? ` · ${org.phone}` : ""}</div>
    <div class="org-sub" style="margin-top:4px;">Printed: ${printDate}</div>
  </div>
</div>
...
<div class="meta-row">
  ...
  <div class="meta-block">
    ...
    ${tenant.email ? `<div class="value-sub">${tenant.email}</div>` : ""}
  </div>
  <div class="meta-block">
    <div class="label">Period / الفترة</div>
    <div class="value">${periodLabel}</div>
    <div class="value-sub">${finalRows.length} transaction${finalRows.length !== 1 ? "s" : ""}</div>
  </div>
</div>
...
<div class="summary-row">
  <div class="summary-box charged">
    ...
    <div class="s-sub">OMR · ${invoices.length} invoice${invoices.length !== 1 ? "s" : ""}</div>
  </div>
  <div class="summary-box paid">
    <div class="s-label">Total Paid</div>
    <div class="s-value">${totalPaid.toFixed(3)}</div>
    <div class="s-sub">OMR · ${payments.length} payment${payments.length !== 1 ? "s" : ""}</div>
  </div>
  <div class="summary-box returned">
    <div class="s-label">Total Returned</div>
    <div class="s-value">${totalReturned.toFixed(3)}</div>
    <div class="s-sub">OMR · ${refunds.length} refund${refunds.length !== 1 ? "s" : ""}</div>
  </div>
  <div class="summary-box balance">
    <div class="s-label">Balance Due</div>
    <div class="s-value">${currentBalance.toFixed(3)}</div>
    <div class="s-sub">OMR · ${currentBalance > 0 ? "Outstanding" : currentBalance < 0 ? "Credit" : "Settled ✓"}</div>
  </div>
</div>
...
<table>
  <thead>
    <tr>
      ...
      <th class="num" style="width:85px">Charges (OMR)</th>
      <th class="num" style="width:85px">Payments (OMR)</th>
      <th class="num" style="width:90px">Balance (OMR)</th>
    </tr>
  </thead>
  <tbody>
    ${finalRows.length === 0
      ? `<tr><td colspan="7" style="text-align:center;padding:20px;color:#9ca3af;">No transactions for this period.</td></tr>`
      : finalRows.map((row) => `
    <tr>
      <td style="white-space:nowrap">${fmtDate(row.date)}</td>
      <td><span class="badge badge-${row.type.toLowerCase()}">${row.type}</span></td>
      <td>${row.description}</td>
      <td style="color:#6b7280;font-size:8pt">${row.reference}</td>
      <td class="num">${row.debit > 0 ? `<span class="debit">${row.debit.toFixed(3)}</span>` : '<span style="color:#d1d5db">—</span>'}</td>
      <td class="num">${row.credit > 0 ? `<span class="credit">${row.credit.toFixed(3)}</span>` : '<span style="color:#d1d5db">—</span>'}</td>
      <td class="num ${row.balance > 0 ? "bal-pos" : "bal-zero"}">${row.balance.toFixed(3)}</td>
    </tr>`).join("")}
  </tbody>
  <tfoot>
    <tr>
      <td colspan="4">TOTALS — ${finalRows.length} transaction${finalRows.length !== 1 ? "s" : ""}</td>
      <td class="num">${totalCharged.toFixed(3)}</td>
      <td class="num">${totalPaid.toFixed(3)}</td>
      <td class="num">${currentBalance.toFixed(3)} OMR</td>
    </tr>
  </tfoot>
</table>
...
<div class="page-footer">
  <span>${org?.name ?? "PMS"} · Salalah, Oman</span>
  <span>Generated ${printDate} · Confidential document</span>
</div>

<script>
  // Auto-focus print dialog when opened in new tab
